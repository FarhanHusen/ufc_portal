...
function getData() {
    const option = {
        ...
        headers: {
            "x-rapidapi-key":
                "aaf9d55d6dmshd6a2dc87e648964p11f8a8jsn5533666c75b0",
...
window.addEventListener("scroll", () => {
    ...
    navLinks.forEach((links) =>
        links.classList.toggle("linkColors", scrollY > 500)
    );
...
function removeStyle() {
    ...
    closeBtnsLIne.forEach(
        (line) => (line.style.transform = "rotate(0) translateY(0)")
    );
    mainMenu.style.left = "-100%";
...
const speed = 2000; // Kecepatan dalam milidetik
let hasAnimated = false; // Untuk memastikan animasi hanya dijalankan sekali

const updateCount = (counter) => {
    const target = +counter.getAttribute("data-target");
    let count = 0; // Mulai dari 0

    const increment = Math.ceil((target / speed) * 100); // Hitung increment per pembaruan

    const interval = setInterval(() => {
        count += increment;
        if (count >= target) {
            count = target; // Jangan lebih dari target
            clearInterval(interval);
        }
        counter.innerText = count;
    }, 10); // Pembaruan setiap 10 ms
};

window.addEventListener("scroll", () => {
    if (scrollY > 350 && !hasAnimated) {
        hasAnimated = true; // Tandai bahwa animasi telah dijalankan
        scoreNums.forEach(updateCount); // Jalankan fungsi updateCount untuk setiap counter
    }
});
